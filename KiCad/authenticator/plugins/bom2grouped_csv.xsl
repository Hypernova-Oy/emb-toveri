...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:key name="partTypeByValueAndFootprint" match="comp" use="./fields/field[@name='Model']" />
	...
	<xsl:template match="components">
	    <!-- for Muenchian grouping of footprint and value combination -->
	    <xsl:for-each select="comp[count(. | key('partTypeByValueAndFootprint', ./fields/field[@name='Model'])[1]) = 1]">
		<xsl:sort select="@ref" />

		<xsl:text>&nl;</xsl:text>
		<!-- list of all references -->
		<xsl:for-each select="key('partTypeByValueAndFootprint', ./fields/field[@name='Model'])">
			<xsl:sort select="@ref" />
			<xsl:value-of select="@ref"/><xsl:text> </xsl:text>
		</xsl:for-each><xsl:text>,</xsl:text>
		<!-- quantity of parts with same footprint and value -->
		<xsl:value-of select="count(key('partTypeByValueAndFootprint', ./fields/field[@name='Model']))"/><xsl:text>,</xsl:text>
        <xsl:text>"</xsl:text>
